...
namespace GremlinNetSample
{
    ...
    class Program
    {
        ...
        private static string Host = "placovugremlinapi.gremlin.cosmos.azure.com";
        private static string PrimaryKey = "6kDqfxJqYqZZq6NZHMj60AKIgg4DZC55o3cKWsmwTL7cFRjwaIyPJhwFXTED94WqzP38jOYAW3Ms56Fy8nMMKA==";
        private static string Database = "placovu-database";
        private static string Container = "placovu-graph";
        ...
        {
            { "Cleanup",        "g.V().drop()" },
            { "AddVertex 1",    "g.addV('person').property('id', 'rasel').property('firstName', 'Rasel').property('age', 36).property('pk', 'pk')" },
            { "AddVertex 2",    "g.addV('person').property('id', 'ayaan').property('firstName', 'Ayaan').property('lastName', 'Ahmmed').property('age', 2).property('pk', 'pk')" },
            { "AddVertex 3",    "g.addV('person').property('id', 'jamal').property('firstName', 'Jamal').property('lastName', 'Mia').property('pk', 'pk')" },
            { "AddVertex 4",    "g.addV('person').property('id', 'sohel').property('firstName', 'Sohel').property('lastName', 'Ahmmed').property('pk', 'pk')" },
            { "AddEdge 1",      "g.V('rasel').addE('knows').to(g.V('ayaan'))" },
            { "AddEdge 2",      "g.V('rasel').addE('knows').to(g.V('jamal'))" },
            { "AddEdge 3",      "g.V('jamal').addE('knows').to(g.V('sohel'))" },
            { "UpdateVertex",   "g.V('rasel').property('age', 36)" },
            { "CountVertices",  "g.V().count()" },
            { "Filter Range",   "g.V().hasLabel('person').has('age', gt(40))" },
            { "Project",        "g.V().hasLabel('person').values('firstName')" },
            { "Sort",           "g.V().hasLabel('person').order().by('firstName', decr)" },
            { "Traverse",       "g.V('rasel').out('knows').hasLabel('person')" },
            { "Traverse 2x",    "g.V('rasel').out('knows').hasLabel('person').out('knows').hasLabel('person')" },
            { "Loop",           "g.V('rasel').repeat(out()).until(has('id', 'sohel')).path()" },
            { "DropEdge",       "g.V('rasel').outE('knows').where(inV().has('id', 'ayaan')).drop()" },
            { "CountEdges",     "g.E().count()" },
            { "DropVertex",     "g.V('rasel').drop()" },
        };
        // </defineQueries>
